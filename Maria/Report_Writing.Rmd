...
We fitted three Bayesian hierarchical models to assess crime risk over space and time. Model 1 includes spatial random effects using the BYM2 specification to capture district-level variation. Model 2 extends this by adding a temporal effect $\gamma_t \sim \text{RW1}$. Model 3 further incorporates a spatial-temporal interaction term $\delta_{it} \sim \mathcal{N}(0, \sigma_\delta^2)$ to allow for district-specific temporal trends.
...
For area $i$ at time $t$, the number of observed cases is denoted by $O$, and the expected number based on the population is denoted by $E$. The incidence rate of the crime (rape/dowry death) is represented by $\rho$. The spatial effect $b_i$ is modeled using the BYM2 parameterization:
\[
b_i = \frac{1}{\sqrt{\tau_b}} \left( \sqrt{1 - \phi} \cdot v_i + \sqrt{\phi} \cdot u_i \right),
\]
where $u_i$ represents the spatially structured component with an intrinsic conditional autoregressive (ICAR) prior, and $v_i$ is the unstructured random effect assumed to follow a normal distribution. Temporal variation is captured by a time effect $\gamma_t$, modeled as a first-order random walk (RW1). In Model 3, $\delta_{it}$ is a spatial-temporal interaction term, assumed to follow a normal distribution with mean zero.
...
## Sensitivity analysis table (title placeholder)

```{r dist_table, fig.width=12, fig.height=12, fig.cap="Table 3: Sensitivity analysis results"}
sensitivity
```
